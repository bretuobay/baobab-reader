...
async function parsePdfToPartialEbook(
  base64Pdf: string,
  filename: string
): Promise<Omit<Ebook, "id" | "uploadedAt" | "tags" | "filePath" | "lastReadPage">> {
  ...
  const extractedMetadata = {
    ...
    subject: getMetaString(() => pdfDoc.getSubject()),
    keywords: getMetaString(() => pdfDoc.getKeywords())?.split(',').map(k => k.trim()).filter(k => k) || undefined,
...
export const pdfRouter = router({
  uploadPdf: publicProcedure
    ...
    .mutation(async ({ input }) => {
      let partialEbookData: Omit<Ebook, "id" | "uploadedAt" | "tags" | "filePath" | "lastReadPage">;
      try {
        partialEbookData = await parsePdfToPartialEbook(input.data, input.filename);
      } catch (error) {
        console.error(`Error parsing PDF metadata for ${input.filename}: ${error instanceof Error ? error.message : String(error)}`);
      ...
      try {
        ...
        console.error(`Error saving Ebook metadata for ${input.filename}: ${error instanceof Error ? error.message : String(error)}`);
