...
import { insertPdfMetadata, getAllPdfMetadata, type PdfMetadata } from '../../db'; // Correct path to db

// Helper function to parse PDF metadata
async function parsePdfMetadata(base64Pdf: string, filename: string): Promise<Omit<PdfMetadata, 'id' | 'uploadedAt'>> {
  ...
  const getMetaString = (metaFunc: () => string | undefined): string | null => {
    try {
      ...
      console.warn(`Warning: Could not extract metadata for ${filename}: ${error instanceof Error ? error.message : String(error)}`);
      return null;
    }
  };
  ...
  const getMetaDate = (metaDateFunc: () => Date | undefined): string | null => {
    try {
      ...
      return date ? date.toISOString() : null;
       ...
       console.warn(`Warning: Could not extract date metadata for ${filename}: ${error instanceof Error ? error.message : String(error)}`);
      return null;
    }
  };
...
export const pdfRouter = router({
  uploadPdf: publicProcedure
    .input(z.object({ filename: z.string(), data: z.string() /* base64 encoded PDF */ }))
